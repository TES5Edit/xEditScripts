...
begin
	...
	// File Dialog	
	if MessageDlg('Create a new patch file [YES] or modify existing plugin [NO]?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
		PatchMode := 1
	else
		PatchMode := 0;
	...
	if patchmode = 0 then begin
		...
		if not InputQuery('Enter', 'Load Order Index', s) then begin
			Result := 1;
			exit;
		end;
		...
		AddMessage('-------------------------------------------------------------------------------');
		AddMessage('Processing:	' + GetFileName(FileByIndex(loadorder + 1)));
		AddMessage('-------------------------------------------------------------------------------');
		AddMessage('');
