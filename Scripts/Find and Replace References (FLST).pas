...
function Initialize: integer;
	...
	begin
		...
		flstFile				:= FileByLoadOrder(StrToInt('$06'));
		flstRefsRecOrigin 		:= RecordByFormID(flstFile, $06FF149A, false);
		flstRefsRecDestination  := RecordByFormID(flstFile, $06FF149B, false);
			...
			if CompareStr(slRefsOrigin[j], name) = 0 then begin
				...
				if CompareStr(filenameProcessing, filename) <> 0 then begin
					...
					try
						...
						if CompareStr(slRefsOrigin[j], name) = 0 then begin
							SetElementEditValues(r, 'NAME', slRefsDestination[j]);
				...
				if CompareStr(filenameProcessing, filename) = 0 then begin
					...
					try
						...
						// Change reference name
						if CompareStr(slRefsOrigin[j], name) = 0 then begin
							SetElementEditValues(e, 'NAME', slRefsDestination[j]);
							slRefsUpdated.Add('Success: ' + formid);
						end;
...
function Finalize: integer;
	...
	begin
		...
		if slRefsUpdated.Count > limit then begin
			...
			dlgSave.FileName := 'fireundubh\[' + Delete(flstFileName, flstFileNameLength, 4) + '] ' + filename + ' - Find and Replace References (Successes).txt';
		...
		if slRefsFailed.Count > limit then begin
			...
			dlgSave.Filter := 'Text files (*.txt)|*.txt';
			dlgSave.InitialDir := ProgramPath;
			dlgSave.FileName := 'fireundubh\[' + Delete(flstFileName, flstFileNameLength, 4) + '] ' + filename + ' - Find and Replace References (Failures).txt';
			if dlgSave.Execute then
